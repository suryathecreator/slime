#!/usr/bin/env python3
"""Publish compact immutable provenance from a completed Nous AIME run."""
...
from examples.qwen2_5_7b_nous_aime_mixed_outcome_sft.data_utils import VARIANTS
...
def render_training_summary(status: dict[str, Any], metrics: dict[str, Any]) -> str:
    ...
    for item in metrics["variants"]:
        ...
        rows.append(
            "| {variant} | {first:.6f} | {last:.6f} | {change:.6f} | {minimum:.6f} | {step} | "
...
def main() -> None:
    ...
    if (
        ...
        or status.get("comparison_checkpoint_count") != 5
        ...
        atomic_json(
            staging / "PROVENANCE_PUBLICATION.json",
            {
                ...
                "excluded_large_payloads": {
                    ...
                    "raw_nous_generations": "excluded from Git; source revision and hashes are recorded",
    ...
    print(
        f"NOUS_AIME_PROVENANCE_PUBLISHED files={len(published_files) + 1} " f"contract={args.contract_hash}",
        flush=True,
    )
