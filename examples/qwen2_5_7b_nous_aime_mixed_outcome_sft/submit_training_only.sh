...
echo "NOUS_AIME_SBATCH_TEST_ONLY_OK jobs=10"

temporary_root="$(mktemp -d "/tmp/${USER:-suryadv}.nous-aime-submit.XXXXXX")"
...
if [[ "${MODE}" == "--dry-run" ]]; then
  dry_manifest="/tmp/${USER:-suryadv}.nous-aime-${CONTRACT_HASH}-dry-run.json"
  ...
  echo "NOUS_AIME_TRAINING_DRY_RUN_OK manifest=${dry_manifest} jobs=10"
  exit 0
fi
...
submit_job() {
  ...
  python3 "${PACKAGE}/write_submission_manifest.py" \
    ...
    --repo-root "${SLIME_REPO_ROOT}" --expected-commit "${submit_commit}" \
    --mode in_progress --replace >/dev/null
  echo "NOUS_AIME_JOB_SUBMITTED name=${name} id=${job_id} dependency=${dependency:-none} variant=${variant:-none}"
}
...
trap - ERR INT TERM
echo "NOUS_AIME_TRAINING_CHAIN_SUBMITTED first=${submitted_ids[0]} final=${submitted_ids[-1]} jobs=${#submitted_ids[@]} contract=${CONTRACT_HASH}"
