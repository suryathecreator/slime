...
# The dependent CPU preflight performs the full package and vLLM import probe.
# Keep submission itself login-node-light while retaining the same DAG gate.
...
if [[ "$MODE" == --test-only-shapes ]]; then
  ...
  echo "Q25R_MATH500_SLURM_SHAPES_VALID targets=6 tasks=72 canary_cases=1"
  exit 0
fi
...
for target in "${TARGETS[@]}"; do
  slug="${target//_/-}"
  submit_job "${COMMON[@]}" --dependency="afterok:$canary_job" --job-name="q25r-m5-$slug" \
  ...
  array_job="$LAST_JOB"
  submit_job "${COMMON[@]}" --dependency="afterok:$array_job" --job-name="q25r-m5-$slug-final" \
...
submit_job "${COMMON[@]}" --dependency="$dependency" --job-name=q25r-m500-audit --cpus-per-task=8 --mem=64G --time=02:00:00 --output="$LOG_DIR/%x-%j.out" "$HANDOFF/audit_cpu.sbatch"
audit_job="$LAST_JOB"
...
echo "Q25R_MATH500_SUBMITTED commit=$git_commit preflight=$preflight_job canary=$canary_job arrays=${array_jobs[*]} finalizers=${finalizer_jobs[*]} audit=$audit_job"
