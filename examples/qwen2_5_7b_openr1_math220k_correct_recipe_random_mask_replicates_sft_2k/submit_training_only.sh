...
for batch_script in \
  01_build_masks.sbatch 02_prefix_canary.sbatch 03_train.sbatch \
  04_finalize.sbatch; do
  sbatch --test-only --chdir="${SLIME_REPO_ROOT}" --account="${ACCOUNT}" \
    --partition="${PARTITION}" --qos="${QOS}" "${SCRIPT_DIR}/${batch_script}"
done
echo "SBATCH_PREFLIGHT_COMPLETE scripts=4"

if [[ "${MODE}" == "--dry-run" ]]; then
  echo "DRY_RUN_SERIAL_AFTEROK name=build_masks"
  ...
  echo "DRY_RUN_COMPLETE jobs=9 maximum_concurrent_gpus=4 eval_jobs=0 transfer_jobs=0"
  exit 0
fi
...
submit_serial build_masks "${SCRIPT_DIR}/01_build_masks.sbatch"
submit_serial prefix_canary "${SCRIPT_DIR}/02_prefix_canary.sbatch"
for variant in "${VARIANTS[@]}"; do
  submit_serial "train_${variant}" \
    --job-name="q25rr-${variant}" --export=ALL,SFT_VARIANT="${variant}" \
