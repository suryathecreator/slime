#!/usr/bin/env python3
"""Nonblocking copy diagnostic using the experiment's exact no-system prompt."""
...
def main() -> None:
    ...
    for problem in problems:
        ...
        rendered_text = problem["rendered_prompt"]
    ...
    atomic_json(
        args.output,
        {
            ...
            "phase": args.phase,
            "prompt_policy": "source_rendered_prompt_verbatim_one_user_no_system",
            "rows": rows,
        },
    )
    print(
        "AIME_SPLIT_COPY_DIAGNOSTIC_COMPLETE "
        f"phase={args.phase} copy_starts="
        f"{sum(row['prompt_copy_start'] or row['system_copy_start'] for row in rows)}/2",
        flush=True,
    )
