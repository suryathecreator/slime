...
REMOTE_EXPERIMENT_RELATIVE="checkpoints/qwen2_5_7b_aime_2009_2024_split_sft_6k/v1/${CONTRACT_HASH}"
...
for required in \
  ...
  "${HELD_IN_EVAL_JSONL}" "${HELD_OUT_EVAL_JSONL}" \
...
local_verify() {
  for record in "${records[@]}"; do
    ...
    echo "AIME_SPLIT_6K_RSYNC_check artifact=${artifact_id}"
  done
...
if [[ "${MODE}" == "--check" ]]; then
  local_verify
  echo "AIME_SPLIT_6K_RSYNC_COMPLETE mode=--check trained=4 eval_jsonl=2 training_jsonl=0 optimizer_state=0"
  exit 0
fi

if [[ "${MODE}" == "--transfer" ]]; then
  local_verify
  control_dir="$(mktemp -d /tmp/q25a6k-rsync-ssh.XXXXXX)"
  ...
  echo "AIME_SPLIT_6K_RSYNC_TRANSFER_SESSION_OPEN authentication_prompts=1"
  remote_directories=(
  ...
  for record in "${records[@]}"; do
    ...
    echo "AIME_SPLIT_6K_RSYNC_transfer artifact=${artifact_id}"
  done
  rsync -a --partial --append-verify --info=progress2 -e "${rsync_rsh}" \
  ...
  if "${ssh_session[@]}" "${REMOTE_HOST}" \
    ...
    echo "AIME_SPLIT_6K_RSYNC_reuse artifact=base_qwen2_5_7b verified=1"
  else
    rsync -a --delete-delay --partial --append-verify --info=progress2 -e "${rsync_rsh}" \
      "${STUDENT_HF_DIR}/" "${REMOTE_HOST}:${REMOTE_SHARED_BASE}/"
    echo "AIME_SPLIT_6K_RSYNC_transfer artifact=base_qwen2_5_7b"
  fi
  for record in "${compact_records[@]}"; do
  ...
  cleanup_session
  trap - EXIT
  echo "AIME_SPLIT_6K_RSYNC_COMPLETE mode=--transfer authenticated_sessions=1 trained=4 eval_jsonl=2 training_jsonl=0 optimizer_state=0 verification=run_--verify"
  exit 0
fi

verify_output="$(mktemp /tmp/q25a6k-rsync-verify.XXXXXX)"
...
for record in "${compact_records[@]}"; do
  ...
  grep -Fqx -- "$(printf 'AIME_SPLIT_6K_REMOTE_SHA256\t%s\t%s' "${expected}" "${relative}")" \
    "${verify_output}" || { echo "Remote hash mismatch: ${relative}" >&2; exit 1; }
done
rm -f -- "${verify_output}"
trap - EXIT
echo "AIME_SPLIT_6K_RSYNC_COMPLETE mode=--verify authenticated_sessions=1 trained=4 eval_jsonl=2 training_jsonl=0 optimizer_state=0"
