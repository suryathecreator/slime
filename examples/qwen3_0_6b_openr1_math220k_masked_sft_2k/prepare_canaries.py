#!/usr/bin/env python3
"""Freeze matching 32-row custom-token and stock-message canary datasets."""
...
from examples.qwen2_5_7b_openr1_math220k_masked_sft_2k.data_utils import atomic_jsonl
...
def trace_id(row: dict) -> str:
    ...
    return str(metadata.get("trace_id") or str(row["id"]).split("/", 1)[-1])
...
def main() -> None:
    ...
    message_ids = [trace_id(row) for row in messages]
