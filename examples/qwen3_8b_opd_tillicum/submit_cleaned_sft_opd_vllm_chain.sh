...
# Tillicum's normal QOS currently caps walltime at 24h. Keep this at the
# largest runnable value; chunked eval artifacts make eval retries resumable.
export CLEANED_WALLTIME_BASE_EVAL="${CLEANED_WALLTIME_BASE_EVAL:-24:00:00}"
export CLEANED_WALLTIME_SFT="${CLEANED_WALLTIME_SFT:-24:00:00}"
export CLEANED_WALLTIME_SFT_EVAL="${CLEANED_WALLTIME_SFT_EVAL:-24:00:00}"
export CLEANED_WALLTIME_OPD1="${CLEANED_WALLTIME_OPD1:-24:00:00}"
export CLEANED_WALLTIME_OPD1_EVAL="${CLEANED_WALLTIME_OPD1_EVAL:-24:00:00}"
export CLEANED_WALLTIME_OPD5="${CLEANED_WALLTIME_OPD5:-24:00:00}"
export CLEANED_WALLTIME_OPD5_EVAL="${CLEANED_WALLTIME_OPD5_EVAL:-24:00:00}"
...
export VLLM_EVAL_GPU_MEMORY_UTILIZATION="${VLLM_EVAL_GPU_MEMORY_UTILIZATION:-0.97}"
export VLLM_EVAL_MAX_NUM_SEQS="${VLLM_EVAL_MAX_NUM_SEQS:-24}"
export VLLM_EVAL_MAX_NUM_BATCHED_TOKENS="${VLLM_EVAL_MAX_NUM_BATCHED_TOKENS:-16384}"
export VLLM_EVAL_KV_CACHE_DTYPE="${VLLM_EVAL_KV_CACHE_DTYPE:-auto}"
export VLLM_EVAL_ENABLE_PREFIX_CACHING="${VLLM_EVAL_ENABLE_PREFIX_CACHING:-0}"
export VLLM_EVAL_ENABLE_CHUNKED_PREFILL="${VLLM_EVAL_ENABLE_CHUNKED_PREFILL:-1}"
export VLLM_EVAL_ASYNC_SCHEDULING="${VLLM_EVAL_ASYNC_SCHEDULING:-1}"
export VLLM_EVAL_SAFETENSORS_LOAD_STRATEGY="${VLLM_EVAL_SAFETENSORS_LOAD_STRATEGY:-prefetch}"
export VLLM_EVAL_SPECULATIVE_METHOD="${VLLM_EVAL_SPECULATIVE_METHOD:-none}"
export VLLM_EVAL_NUM_SPECULATIVE_TOKENS="${VLLM_EVAL_NUM_SPECULATIVE_TOKENS:-0}"
export VLLM_EVAL_PROMPT_LOOKUP_MIN="${VLLM_EVAL_PROMPT_LOOKUP_MIN:-5}"
export VLLM_EVAL_PROMPT_LOOKUP_MAX="${VLLM_EVAL_PROMPT_LOOKUP_MAX:-5}"
...
echo "vLLM eval memory/KV/scheduler/speculative: ${VLLM_EVAL_GPU_MEMORY_UTILIZATION}/${VLLM_EVAL_KV_CACHE_DTYPE}/${VLLM_EVAL_ASYNC_SCHEDULING}/${VLLM_EVAL_SPECULATIVE_METHOD}:${VLLM_EVAL_NUM_SPECULATIVE_TOKENS}"
echo "vLLM eval chunk resume: chunk_size=${VLLM_EVAL_CHUNK_SIZE} resume_completed=${VLLM_EVAL_RESUME_COMPLETED}"
echo "Qwen3 thinking/stops: enable_thinking=${QWEN3_ENABLE_THINKING} stop_token_ids=${QWEN3_STOP_TOKEN_IDS}"
echo "vLLM eval preserves special tokens/raw ids: ${VLLM_EVAL_PRESERVE_SPECIAL_TOKENS}"
...

{
  ...
  echo "qwen3_enable_thinking=${QWEN3_ENABLE_THINKING}"
  echo "qwen3_stop_token_ids=${QWEN3_STOP_TOKEN_IDS}"
  echo "vllm_eval_preserve_special_tokens=${VLLM_EVAL_PRESERVE_SPECIAL_TOKENS}"
