...
jid_opd1=$(sbatch --parsable "${SBATCH_GPU4[@]}" --dependency="afterok:${jid_base_report}" --time=24:00:00 --job-name=q3-4b-opd-001024 --export=ALL,OPD_STAGE=1k examples/qwen3_4b_opd_harp/03_run_opd.sbatch)
jid_eval1=$(sbatch --parsable "${SBATCH_GPU4[@]}" --dependency="afterok:${jid_opd1}" --time=24:00:00 --job-name=q3-4b-harp-opd1k --export="ALL,EVAL_MODE=eval,EVAL_MODEL_DIR=${OPD1_HF_DIR}/iter_0000007,EVAL_TOKENIZER_DIR=${STUDENT_HF_DIR},EVAL_STAGE_DIR=${EVAL_OPD1K_DIR}/shards,EVAL_MAX_NUM_SEQS=${VLLM_EVAL_4B_MAX_NUM_SEQS},EVAL_PREFER_CUDA_GRAPH=${VLLM_EVAL_PREFER_CUDA_GRAPH}" examples/qwen3_4b_opd_harp/01_eval_harp_vllm.sbatch)
jid_report1=$(sbatch --parsable "${SBATCH_REPORT[@]}" --dependency="afterok:${jid_eval1}" --time=01:00:00 --job-name=q3-harp-opd1k-report --export=ALL,REPORT_MODE=opd1k examples/qwen3_4b_opd_harp/04_report_harp.sbatch)
jid_opd5=$(sbatch --parsable "${SBATCH_GPU4[@]}" --dependency="afterok:${jid_report1}" --time=24:00:00 --job-name=q3-4b-opd-005120 --export=ALL,OPD_STAGE=5k examples/qwen3_4b_opd_harp/03_run_opd.sbatch)
