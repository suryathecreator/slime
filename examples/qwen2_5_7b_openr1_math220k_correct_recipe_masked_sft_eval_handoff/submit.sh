...
if [[ "$MODE" == --test-only-shapes ]]; then
  ...
  sbatch --test-only "${COMMON[@]}" --array=0-11 --gpus=h200:1 --exclude=g3130 --export=ALL,EVAL_TARGET=unmasked --output="$LOG_DIR/%x-%A_%a.out" "$HANDOFF/run_h200.sbatch"
  sbatch --test-only "${COMMON[@]}" --export=ALL,EVAL_TARGET=unmasked,EVAL_GIT_COMMIT=0000000000000000000000000000000000000000 --output="$LOG_DIR/%x-%j.out" "$HANDOFF/finalize_cpu.sbatch"
  sbatch --test-only "${COMMON[@]}" --output="$LOG_DIR/%x-%j.out" "$HANDOFF/audit_cpu.sbatch"
  echo "Q25M_MATH500_SLURM_SHAPES_VALID targets=10 tasks=120 canary_cases=1"
  exit 0
fi
...
for target in "${TARGETS[@]}"; do
  slug="${target//_/-}"
  submit_job "${COMMON[@]}" --dependency="afterok:$canary_job" --job-name="q25m-m5-$slug" \
    --array=0-11 --gpus=h200:1 --exclude=g3130 --cpus-per-task=8 --mem=96G --time=02:00:00 \
    --signal=B:USR1@600 --export=ALL,EVAL_TARGET="$target" \
    --output="$LOG_DIR/%x-%A_%a.out" "$HANDOFF/run_h200.sbatch"
  array_job="$LAST_JOB"
  submit_job "${COMMON[@]}" --dependency="afterok:$array_job" --job-name="q25m-m5-$slug-final" \
...
submit_job "${COMMON[@]}" --dependency="$dependency" --job-name=q25m-m500-audit --cpus-per-task=8 --mem=64G --time=02:00:00 --output="$LOG_DIR/%x-%j.out" "$HANDOFF/audit_cpu.sbatch"
audit_job="$LAST_JOB"
...
echo "Q25M_MATH500_SUBMITTED commit=$git_commit preflight=$preflight_job canary=$canary_job arrays=${array_jobs[*]} finalizers=${finalizer_jobs[*]} audit=$audit_job"
