...
readonly FAILED_SMOKE_JOB=199393
readonly BLOCKED_SCORE_JOB=198097
readonly FIRST_TRAIN_JOB=198098
readonly TAIL_TRAIN_JOB=198108
readonly PRIOR_REPAIR_MANIFEST="${MANIFEST_ROOT}/smoke_validator_packaging_repair.json"
readonly FAILED_LOG="${SLURM_LOG_DIR}/q25-7b-smoke-r2_${FAILED_SMOKE_JOB}.log"
readonly FAILED_STDOUT="${SLIME_REPO_ROOT}/slurm-${FAILED_SMOKE_JOB}.out"
readonly REPAIR_STEM="smoke_start_rollout_repair"
...
readonly ROLLOUT_DATA="${FAILED_OUTPUT}/details/rollout_data/1.pt"
readonly EXPECTED_PARTIAL_DATA_SHA256="78d2ba09a34bd386a7bb17c17d2071bdacb85a185e6be2c06b1b827870d26d9a"
readonly EXPECTED_ROLLOUT_DATA_SHA256="49aa00bacef54469e21f9458173633b14302356cd63ba17192cd65a7e9a596b8"
...
[[ -f "${FAILED_LOG}" && -f "${FAILED_STDOUT}" ]] || {
  echo "Missing failed replacement smoke logs." >&2
  exit 1
}
grep -Fq "SFT chat template kwargs: {}" "${FAILED_LOG}" || {
  echo "Failed smoke did not use the corrected Qwen2.5 JSON value." >&2
  exit 1
}
grep -Fq "perf 1:" "${FAILED_LOG}" || {
  echo "Failed smoke did not generate the observed rollout 1." >&2
  exit 1
}
grep -Fq "succeeded" "${FAILED_LOG}" || {
  echo "Failed smoke did not complete its Ray entrypoint." >&2
  exit 1
}
if grep -Fq "Timer train start" "${FAILED_LOG}"; then
  echo "Failed smoke unexpectedly entered an optimizer step." >&2
  exit 1
fi
grep -Fq "CHECKPOINT_TMPDIR_PREFLIGHT_OK" "${FAILED_STDOUT}" || {
  echo "Failed smoke did not pass its checkpoint TMPDIR preflight." >&2
  exit 1
}
...
if manifest["status"] != "submitted_and_rewired":
    raise RuntimeError("smoke packaging repair is not finalized")
if manifest["replacement_smoke"] != 199393:
    raise RuntimeError("prior replacement smoke drift")
if manifest["rewired_dependency"] != {
    "job_id": 198097,
    "dependency": "afterok:199393",
}:
    raise RuntimeError("prior repaired score dependency drift")
PY
...
[[ -d "${FAILED_OUTPUT}" && -f "${PARTIAL_DATA}" && -f "${ROLLOUT_DATA}" ]] || {
  echo "Missing failed smoke rollout output: ${FAILED_OUTPUT}" >&2
  exit 1
}
[[ "$(find "${FAILED_OUTPUT}" -type f | wc -l)" == "5" ]] || {
  echo "Failed smoke output inventory drifted; refusing repair." >&2
  exit 1
}
if find "${FAILED_OUTPUT}/full_state" "${FAILED_OUTPUT}/weights" -type f -print -quit | grep -q .; then
  echo "Failed smoke unexpectedly contains a checkpoint." >&2
  exit 1
fi
...
[[ "$(sha256sum "${ROLLOUT_DATA}" | awk '{print $1}')" == "${EXPECTED_ROLLOUT_DATA_SHA256}" ]] || {
  echo "Failed smoke rollout hash drifted; refusing repair." >&2
  exit 1
}

export FAILED_LOG_SHA256 FAILED_STDOUT_SHA256 FAILED_OUTPUT_BYTES
export FAILED_FILE_COUNT PARTIAL_DATA_SHA256 ROLLOUT_DATA_SHA256
FAILED_LOG_SHA256="$(sha256sum "${FAILED_LOG}" | awk '{print $1}')"
FAILED_STDOUT_SHA256="$(sha256sum "${FAILED_STDOUT}" | awk '{print $1}')"
...
ROLLOUT_DATA_SHA256="$(sha256sum "${ROLLOUT_DATA}" | awk '{print $1}')"
export REPAIR_COMMIT
REPAIR_COMMIT="$(git rev-parse HEAD)"
mkdir -p "${MANIFEST_ROOT}" "$(dirname -- "${FAILED_ARCHIVE}")"

export REPAIR_PENDING REPAIR_FAILED FAILED_ARCHIVE FAILED_LOG FAILED_STDOUT
export FAILED_OUTPUT PARTIAL_DATA ROLLOUT_DATA COMPLETED_MODEL_JOB COMPLETED_DATA_JOB
export FAILED_SMOKE_JOB BLOCKED_SCORE_JOB FIRST_TRAIN_JOB TAIL_TRAIN_JOB
python3 - <<'PY'
import json
import os
import tempfile
...
value = {
    ...
    "reason": "fresh converted base inferred start rollout 1 and skipped training",
    "jobs": {
    ...
    "failed_stdout": {
        "path": os.environ["FAILED_STDOUT"],
        "sha256": os.environ["FAILED_STDOUT_SHA256"],
    },
    "failed_smoke": {
        ...
        "rollout_data_original": os.environ["ROLLOUT_DATA"],
        "rollout_data_archive": str(
            Path(os.environ["FAILED_ARCHIVE"]) / "details/rollout_data/1.pt"
        ),
        "rollout_data_sha256": os.environ["ROLLOUT_DATA_SHA256"],
        "generated_rollout_id": 1,
        "optimizer_steps": 0,
        "resumable": False,
    },
}
...
[[ "$(sha256sum "${FAILED_ARCHIVE}/details/rollout_data/1.pt" | awk '{print $1}')" == "${ROLLOUT_DATA_SHA256}" ]] || {
  echo "Archived smoke rollout hash mismatch." >&2
  exit 1
}
...
repair_complete=1
trap - EXIT
echo "SMOKE_START_ROLLOUT_REPAIR_COMPLETE failed=${FAILED_SMOKE_JOB} replacement=${replacement_job} rewired=${BLOCKED_SCORE_JOB} reused=198097-198108"
