...
readonly FAILED_SMOKE_JOB=199376
readonly BLOCKED_SCORE_JOB=198097
readonly FIRST_TRAIN_JOB=198098
readonly TAIL_TRAIN_JOB=198108
readonly PRIOR_REPAIR_MANIFEST="${MANIFEST_ROOT}/smoke_json_repair.json"
readonly FAILED_LOG="${SLURM_LOG_DIR}/q25-7b-smoke-r1_${FAILED_SMOKE_JOB}.log"
readonly FAILED_STDOUT="${SLIME_REPO_ROOT}/slurm-${FAILED_SMOKE_JOB}.out"
readonly REPAIR_STEM="smoke_validator_packaging_repair"
...
grep -Fq "syntax error: unexpected end of file" "${FAILED_LOG}" || {
  echo "Smoke log does not contain the approved payload-packaging failure." >&2
  exit 1
}
grep -Fq "CHECKPOINT_TMPDIR_PREFLIGHT_OK" "${FAILED_STDOUT}" || {
  echo "Failed replacement did not pass its checkpoint TMPDIR preflight." >&2
  exit 1
}
...
if manifest["status"] != "submitted_and_rewired":
    raise RuntimeError("smoke JSON repair is not finalized")
if manifest["replacement_smoke"] != 199376:
    raise RuntimeError("prior replacement smoke drift")
if manifest["rewired_dependency"] != {
    "job_id": 198097,
    "dependency": "afterok:199376",
}:
...
[[ -d "${FAILED_OUTPUT}" && -f "${PARTIAL_DATA}" ]] || {
  echo "Missing failed replacement smoke output: ${FAILED_OUTPUT}" >&2
  exit 1
}
[[ "$(find "${FAILED_OUTPUT}" -type f | wc -l)" == "1" ]] || {
  echo "Replacement smoke output inventory drifted; refusing repair." >&2
  exit 1
}
[[ "$(wc -l <"${PARTIAL_DATA}")" == "200" ]] || {
  echo "Replacement smoke subset row count drifted; refusing repair." >&2
  exit 1
}
[[ "$(sha256sum "${PARTIAL_DATA}" | awk '{print $1}')" == "${EXPECTED_PARTIAL_DATA_SHA256}" ]] || {
  echo "Replacement smoke subset hash drifted; refusing repair." >&2
  exit 1
}

export FAILED_LOG_SHA256 FAILED_STDOUT_SHA256 FAILED_OUTPUT_BYTES
export FAILED_FILE_COUNT PARTIAL_DATA_SHA256
...
export FAILED_OUTPUT PARTIAL_DATA COMPLETED_MODEL_JOB COMPLETED_DATA_JOB
...
value = {
    ...
    "reason": "single quotes terminated the container bash -lc payload",
    "jobs": {
...
repair_complete=1
trap - EXIT
echo "SMOKE_PACKAGING_REPAIR_COMPLETE failed=${FAILED_SMOKE_JOB} replacement=${replacement_job} rewired=${BLOCKED_SCORE_JOB} reused=198097-198108"
