...
readonly FAILED_MODEL_JOB=198094
...
readonly FAILED_LOG="${SLURM_LOG_DIR}/q25-7b-model_${FAILED_MODEL_JOB}.log"
readonly REPAIR_MANIFEST="${MANIFEST_ROOT}/model_validation_repair.json"
readonly REPAIR_PENDING="${MANIFEST_ROOT}/model_validation_repair.pending.json"
readonly REPAIR_FAILED="${MANIFEST_ROOT}/model_validation_repair.failed.json"
...
[[ "$(job_state "${FAILED_MODEL_JOB}")" == FAILED* ]] || {
  echo "Expected failed model-preparation job ${FAILED_MODEL_JOB}." >&2
  exit 1
}
[[ -f "${FAILED_LOG}" ]] || {
  echo "Missing failed model-preparation log ${FAILED_LOG}." >&2
  exit 1
}
grep -Fq "invalid literal for int() with base 10: 'input_ids'" "${FAILED_LOG}" || {
  echo "Failed job does not contain the approved BatchEncoding failure." >&2
  exit 1
}
...
export ORIGINAL_MANIFEST FAILED_MODEL_JOB BLOCKED_DATA_JOB NEXT_SMOKE_JOB TAIL_TRAIN_JOB
python3 - <<'PY'
import json
import os
...
expected = list(range(198094, 198109))
...
if manifest["contract_hash"] != "6cfd7bd235e078b8":
    raise RuntimeError("original contract hash drift")
if manifest["commit"] != "0c5cbcbb8532ed6761c67b64c3b2353fcc74c2c5":
    raise RuntimeError("original submission commit drift")
PY

[[ ! -e "${STUDENT_TORCH_DIST_DIR}/latest_checkpointed_iteration.txt" ]] || {
  echo "Base torch-dist conversion unexpectedly completed." >&2
  exit 1
}
...
model = Path(os.environ["STUDENT_HF_DIR"])
shards = sorted(
    [
        {"name": item.name, "bytes": item.stat().st_size}
        for item in model.glob("model-*.safetensors")
    ],
...
value = {
    ...
    "reason": "validate_base_model iterated BatchEncoding keys instead of normalized input_ids",
    "jobs": {
        "failed_model_preparation": 198094,
        "blocked_data_to_rewire": 198095,
        "preserved_downstream_first": 198096,
        "preserved_downstream_last": 198108,
    },
...
repair_complete=1
trap - EXIT
echo "MODEL_VALIDATION_REPAIR_COMPLETE failed=${FAILED_MODEL_JOB} replacement=${replacement_job} rewired=${BLOCKED_DATA_JOB} reused=198095-198108"
