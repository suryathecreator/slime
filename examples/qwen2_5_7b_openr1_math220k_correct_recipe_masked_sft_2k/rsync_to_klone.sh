...
REMOTE_EXPERIMENT="${REMOTE_REPO}/checkpoints/qwen2_5_7b_openr1_math220k_correct_recipe_masked_sft_2k/v1/${CONTRACT_HASH}"
...
[[ "${#records[@]}" -eq 10 ]] || { echo "Expected 10 new handoff records" >&2; exit 1; }
...
if [[ "${MODE}" == "--transfer" ]]; then
  ...
  echo "RSYNC_HANDOFF_METADATA_TRANSFERRED files=7"
fi
