...
def parse_args() -> argparse.Namespace:
    ...
    shard.add_argument(
        ...
        help=(
            "Portable content identity for the checkpoint. When omitted, hash "
            "the local checkpoint before generation."
        ),
    )
...
def policy(
    ...
    return {
        # Deliberately identify the model by content instead of an absolute
        # machine-specific path. The load path remains in each raw artifact as
        # provenance but does not change the scientific policy fingerprint.
        "model_checkpoint_manifest_sha256": checkpoint_manifest_sha256,
...
def build_result(
    *,
    ...
    stop_token_id = (
        ...
        or (isinstance(raw_stop, str) and raw_stop.isdigit())
        else None
    )
...
async def run_pending_async(
    *,
    ...
    async def generate_one(
        item: tuple[int, dict[str, Any], list[int], str, int]
    ) -> None:
        index, row, input_ids, rendered, cap = item
        ...
        async for output in engine.generate(
            ...
            sampling,
            request_id=f"{args.stage}-{index:04d}",
        ):
            final_output = output
        if final_output is None or not final_output.finished:
            raise RuntimeError(f"vLLM request did not finish for eval index {index}")
        result = build_result(
            args=args,
            row=row,
            index=index,
        ...
        target = result_path(Path(args.output_dir), index)
        atomic_text(
            target,
            json.dumps(result, ensure_ascii=False, sort_keys=True) + "\n",
        )
        print(
            f"EVAL_SAMPLE_COMMIT stage={args.stage} index={index} "
            f"correct={int(result['scorer_trace']['is_correct'])} path={target}",
            flush=True,
        )

    try:
        # Submit all requests immediately. vLLM performs continuous batching
        # within max_num_seqs; each task durably commits as soon as it finishes.
...
def run_shard(args: argparse.Namespace) -> None:
    ...
    checkpoint_manifest_sha256 = require_sha256(
        args.checkpoint_manifest_sha256
        or checkpoint_manifest(args.model)["manifest_sha256"],
        label="checkpoint manifest identity",
    )
...
def checkpoint_manifest(model: str) -> dict[str, Any]:
    ...
    if not (root / "config.json").is_file():
        raise RuntimeError(f"Checkpoint is missing config.json: {root}")
    files = []
...
def aggregate(args: argparse.Namespace) -> None:
    ...
    metrics: dict[str, Any] = {
        ...
        "model_checkpoint_manifest_sha256": rows[0]["policy"].get(
    ...
    if model_path:
        ...
        expected_checkpoint = rows[0].get(
            "model_checkpoint_manifest_sha256",
            rows[0]["policy"].get("model_checkpoint_manifest_sha256"),
        )
        if expected_checkpoint and metrics["model_checkpoint"]["manifest_sha256"] != expected_checkpoint:
            raise RuntimeError(
                "Checkpoint content changed between generation and aggregation"
            )
