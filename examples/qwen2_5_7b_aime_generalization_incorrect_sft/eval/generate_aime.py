#!/usr/bin/env python3
"""Run one resumable 100-problem AIME evaluation shard with offline vLLM."""
...
def read_jsonl_recover_truncated_tail(path: Path) -> list[dict[str, Any]]:
    """Recover only an unterminated malformed final append after preemption."""
...
def sample_seed(repeat: int, eval_index: int) -> int:
    if repeat not in range(REPEATS):
        raise ValueError(f"repeat must be 0..{REPEATS - 1}: {repeat}")
    if eval_index not in range(PROBLEMS_PER_SPLIT):
        raise ValueError(f"eval_index must be 0..{PROBLEMS_PER_SPLIT - 1}: {eval_index}")
    return 1234 + PROBLEMS_PER_SPLIT * repeat + eval_index
...
def effective_response_budget(rendered_prompt_tokens: int) -> int:
    ...
    if rendered_prompt_tokens < 1 or budget < 1:
        raise RuntimeError(
            f"prompt leaves no response budget: {TARGET_CONTEXT} - " f"{rendered_prompt_tokens} = {budget}"
        )
    return budget
    ...
    *,
    ...
    return {
        ...
        "prompt": {
            ...
            "input_key": "prompt",
            "messages": "one user message whose content is the transferred prompt verbatim",
        },
        "repeat": repeat,
...
def reconcile_records(path: Path, expected_indices: list[int], policy_sha256: str) -> dict[int, dict[str, Any]]:
    expected = set(expected_indices)
    records: dict[int, dict[str, Any]] = {}
    if path.is_file():
        for row in read_jsonl_recover_truncated_tail(path):
            index = int(row.get("eval_index", -1))
            if index in expected and row.get("policy_sha256") == policy_sha256:
                records[index] = row
...
def run(args: argparse.Namespace) -> None:
    ...
    if not rows or len(rows) > 100:
        raise RuntimeError(f"eval shard must contain 1..100 rows, found {len(rows)}")
    indices = [int(row["eval_index"]) for row in rows]
    if len(indices) != len(set(indices)):
        raise RuntimeError("eval shard contains duplicate eval_index values")
    for row in rows:
        if not isinstance(row.get("prompt"), str) or not row["prompt"].strip():
            raise RuntimeError(f"missing transferred prompt at eval_index={row.get('eval_index')}")
    ...
    if actual_stops != STOP_TOKENS or int(tokenizer.eos_token_id) != 151643:
        raise RuntimeError(
            f"tokenizer stop-token contract changed: actual={actual_stops} " f"eos={tokenizer.eos_token_id}"
        )
    ...
    if not pending:
        print(
            f"AIME_GENERATION_REUSED target={args.target} dataset={args.dataset} " f"repeat={args.repeat}",
            flush=True,
        )
        return
    ...
    for row in pending:
        ...
        prepared.append((row, input_ids, rendered, effective_response_budget(len(input_ids))))
    ...
    try:
        for start in range(0, len(prepared), args.chunk_size):
            ...
            print(
                f"AIME_GENERATION_PROGRESS target={args.target} "
                f"dataset={args.dataset} repeat={args.repeat} "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dataset", required=True, choices=["held_in", "held_out_problem"])
